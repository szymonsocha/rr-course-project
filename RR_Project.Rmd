...
Feature selection involves identifying the optimal feature and establishing a strong relationship with the independent variable.
...
MI scores are sorted in decreasing order in a dataframe as follows:
...
Further, two function for modeling were created based on the *caret* package. `train_predict` function fits the model to the training data within the selected method (LR, KNN, DT, GDB, etc.), make predictions on the test data and returns them.
...
Another Feature Importance technique was proposed using *caret* package - Learning Vector Quantization (LVQ) model.
...
# Train the model
model <- train(y ~ ., data = data.frame(X, y), method = "lm", preProcess = "scale", trControl = control)

# Estimate variable importance
importance <- varImp(model, scale = FALSE)

# Extract variable names and importance scores
variable_names <- rownames(importance$importance)
importance_scores <- importance$importance[, "Overall"]

# Create data frame
importance_df <- data.frame(Variable = variable_names, Importance = importance_scores)
importance_df <- importance_df[order(importance_df$Importance, decreasing = TRUE), ]

# Summarize importance in a datatable
options(max.print = 33)
datatable(importance_df, options = list(dom = 't',
                                        paging = FALSE,
                                        scrollX = TRUE,
                                        scrollCollapse = TRUE),
          rownames = FALSE)
