...
In order to make this project reproducible, we utilize the `renv` package to store the environment state.
...
# To make our project reproducible, we use `renv` package. By utilizing the following command, the appropriate versions of the libraries will be loaded
renv::restore()
```
...
The data structure is somewhat unusual. The departure time is given as a string (hour and minutes separated by a colon). The arrival time format is unstructured, and sometimes includes the date. The flight time contains mixed hours and minutes as a string.
...
Dataset consists of 11 columns and 10683 observations.
...
Let's check the data missings on the bar chart. On the horizontal axis we show the name of the column, on the horizontal axis the number of data missings.
...
We observe two missing data for the `Route` and `Total_Stops` variables.
...
In the previous step, we explored the dataset by detecting two data missings and removing them. In this step, we clean the data, convert the column format and prepare the data for passing to the Machine Learning model.
...
The format of the date column is a string. We are converting the date columns to date format.
...
From the `Date_of_Journey` column, we extract the day. From the `Date_of_Journey` column, we extract the month.
...
We will no longer use the `Date_of_Journey` column, we are removing it.
...
From the `Arrival_time` and `Dept_time` columns, we extract the hour and minutes. We then remove these columns.

We create functions to convert problematic columns containing time data.
...
Using regular expressions, we correct the `Duration` column.

We separate the hours and minutes.
...
Using the sapply function together with the created function, we split the hours and minutes into two separate columns.
...
The dataframe already looks much better, let's still change the data type in the `dur_hour` and `dur_min` columns to integers.
...
For more detailed EDA, below we explore variables by categorical and numerical variables.

#### Finding the categorical value

We save the names of the columns with categorical data
...
We save the names of the columns with numerical data
...
For nominal data, which has no order, we use one hot encoding. For oridinal, sorted data we use lablel encoder
...
Analyzing the chart, we see that Jet Airways Business has the highest price. The others have approximate median prices.
